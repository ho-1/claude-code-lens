...
export class ClaudeTreeProvider implements vscode.TreeDataProvider<TreeItem> {
  ...
  async getChildren(element?: TreeItem): Promise<TreeItem[]> {
    ...
    if (!element) {
      const items: TreeItem[] = this._scanResult.folders.map((folder, index) =>
        this._createFolderTreeItem(folder, index),
      )
    ...
    if (element.contextValue === 'teamItem' && element.teamConfig) {
      return element.teamConfig.members.map((member) => {
        const item = new TreeItem(member.name, vscode.TreeItemCollapsibleState.None)
  ...
  private _createAgentTeamsRoot(): TreeItem {
    const item = new TreeItem('Agent Teams', vscode.TreeItemCollapsibleState.Expanded)
  ...
  private _createTeamItem(team: TeamConfig): TreeItem {
    ...
    const item = new TreeItem(team.name, vscode.TreeItemCollapsibleState.Collapsed)
  ...
  private _createFolderTreeItem(folder: ClaudeFolder, _index: number): TreeItem {
  ...
  private _createItemTreeItem(configItem: ClaudeConfigItem, parentFolder?: string): TreeItem {
    ...
    const item = new TreeItem(title, vscode.TreeItemCollapsibleState.None)
    item.id = itemId
