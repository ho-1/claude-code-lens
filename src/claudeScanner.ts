...
import { scanAgentTeamsSettings, scanTeams } from './teamScanner'
import { ClaudeConfigItem, ClaudeFolder, HooksConfig, ScanResult } from './types'
...
export async function scanWorkspace(): Promise<ScanResult> {
  ...
  if (!workspaceFolders) {
    return {
      ...
      agentTeamsSettings: defaultSettings,
      insights: null,
    }
  }
  ...
  // Scan teams, tasks, and settings in parallel
  const [teams, tasks, agentTeamsSettings] = await Promise.all([
    scanTeams(),
    scanTasks(),
    scanAgentTeamsSettings(),
  ])

  const teamData = { teams, tasks }
...
async function countHooksInFolder(items: ClaudeConfigItem[]): Promise<number> {
  ...
  for (const item of items) {
    if (
      item.type === 'file' &&
      item.name.toLowerCase().startsWith('settings') &&
