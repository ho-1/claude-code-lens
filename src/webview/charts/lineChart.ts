...
export function renderLineChartSvg(config: LineChartConfig): string {
  ...
  for (let i = 0; i < xLabelCount; i++) {
    ...
    xLabels.push(
      `<text x="${x}" y="${height - 5}" text-anchor="middle" fill="var(--vscode-descriptionForeground)" font-size="9">${shortDate}</text>`,
    )
  }

  // Series paths
  const seriesPaths = config.series
    .map((s) => {
  ...
  if (showLegend && config.series.length > 1) {
    const legendItems = config.series
      .map((s, i) => {
        ...
        return (
          `<circle cx="${x}" cy="${height + 15}" r="4" fill="${s.color}"/>` +
